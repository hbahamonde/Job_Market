...
% research questions
\miniframesoff
\begin{frame}[c]\frametitle{Research Question 1}
\begin{center}
\huge{What are the {\bf origins} of the tax state in Latin America?}\pause
\\
\vspace{2cm}
\large{The {\bf emergence of challenger elites}, in the early 20th century, \emph{accelerated} the \emph{expansion} of the \emph{fiscal system}, \emph{fostering} in turn state capacities.}
\end{center}
\end{frame}


\miniframesoff
\begin{frame}[c]\frametitle{Research Question 2}
\begin{center}
\huge{How can we {\bf measure the effects} of fiscal expansion on state capacities overtime?}\pause
\\
\vspace{2cm}
\large{Use a novel hand-collected longitudinal {\bf dataset on earthquakes death tolls} to proxy the \emph{capacity} of the state to enforce building codes at the subnational level.}
\end{center}
\end{frame}
...
% why income tax
\begin{frame}\frametitle{Why Income Taxation}
	...
	\begin{itemize}
		...
		\item Income taxation required the state sending tax collectors to the entire territory, increasing state presence.

			\begin{enumerate}
				\item Developed technologies to monitor individual incomes.
				\item Required trained bureaucracies.
				\item Generated routines and standard procedures: \emph{check accounting books}.
				\item Domestic agreements, especially from the ones carrying the heaviest burden (elites).
			\end{enumerate}
...
\begin{frame}\frametitle{Argument}
	\begin{itemize}
		...
		\item[] In turn, balanced military capacities prevented conflict, fostering inter-sectoral compromises. {\bf Income tax}.
...
\begin{frame}\frametitle{Argument}
	\begin{itemize}
		...
		\item[] Income {\bf taxation} was the {\color{red}{\bf engine} that promoted {\bf state reach}}, fostering {\bf state-building}.
