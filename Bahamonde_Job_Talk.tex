...
\begin{itemize}
	...
	\item Findings and theory from two dissertation chapters (now papers)\pause:
	...
	\begin{enumerate}
		...
		\\
		{\bf {\color{red}Status}}: {\input{/Users/hectorbahamonde/RU/Dissertation/Papers/Earthquake_Paper/status.txt}\unskip}.\pause
...
\begin{itemize}
	\pause\item[] Why did a weaker earthquake flatten Haiti?\pause\;It seems clear that Chile has better ``state capacities'' compared to Haiti.
\end{itemize}
...
\begin{enumerate}
	...
	\item Earthquakes serve as good proxies of state capacities. The {\bf capacity} the state has of {\bf enforcing} quake-sensitive {\bf building codes} throughout the territory is a {\bf {\color{red}reflection}} of its {\bf overall} state-capacities.
\end{enumerate}
...
\begin{frame}\frametitle{Income Taxation and State Capacities}
	...
	\begin{itemize}
		\item[] {\bf Fiscal sociology} theory. Income taxation offers a theory of state formation.

		\item[] Monitoring private incomes, and converting them into {\color{red}public property}, \emph{fostered} state formation.

			\begin{enumerate}
				\item \emph{Indirect} taxes are easier to collect: ex., collect them at ports.
				\item \emph{Direct} taxes are harder to collect: required the state sending tax collectors to the entire territory, increasing state presence.\pause
			\end{enumerate}
		...
		\item[] In simple, the same bureaucracies that were sent to collect the taxes, used the acquired knowledge to perform other state tasks (justice dispensation, security provision, etc.)
	\end{itemize}
...
\begin{frame}\frametitle{Argument}
	\begin{itemize}
		...
		\item[] Also, industrial expansion required more infrastructure (public goods), such as roads and bridges. Hence industrial elites were more willing to pay for those via an income tax.
...
\begin{frame}\frametitle{Argument}
	\begin{itemize}
		...
		\item[] State presence became more dense and complex, fostering state-making. Income {\bf taxation} was the {\color{red}{\bf engine} that promoted {\bf state reach}} and {\bf state-building}.
...
\begin{frame}\frametitle{Argument}
	\begin{itemize}
		...
		\item[] {\color{red}However}, a weak industrial sector engendered weak industrial elites unable to contest the institutional order. There was no need to make inter-sectoral alliances, {\bf delaying the implementation of the income tax}.
...
\begin{frame}[c]\frametitle{Research Question}
\begin{center}
\huge{How can we {\bf measure the effects} of fiscal expansion on state capacities overtime?}\pause
\\
\vspace{2cm}
...
\begin{frame}
	\begin{itemize}
		\item[] The first section helps us to understand \emph{when} countries implement the income tax, and \emph{why} `when' is relevant.\pause\; However, it doesn't link income taxation with \emph{actual} state capacities.\pause%\; Historians and fiscal sociologists provide some answers, but this relationship in the Latin American political economies has been overlooked.\pause\; \emph{{\bf Did the expansion of the fiscal system improve state capacities overtime in Latin America?}}\pause

		\item[\tiny{Mechanism}] \emph{How} did income taxation {\bf expanded} \emph{overall} state capacities? 

		\item[\tiny{Measurement}] \emph{How} can we {\bf measure} the effects of income taxation on state of capacities overtime?

\pause\dotfill

	\item[{\color{red}$\star$}] {\bf Historians} and {\bf fiscal sociologists} provide some answers, but this relationship has been overlooked in the Latin American context.\pause
