...
\begin{frame}\frametitle{{\scriptsize{``\input{/Users/hectorbahamonde/research/Economic_Experiment_Vote_Selling/title.txt}\unskip.}''}}
	...
	%\column{0.5\textwidth}
		\begin{itemize}
			...
			\item {\bf Behavioral economics} framework, {\color{blue}deliberation games}: subjects make decisions and get payed according to the quality of their decisions.
...
\column{0.6\textwidth}
		\begin{itemize}
			...
			\item {\bf Hypothesis}: the poor bore the bore the cost of the COVID pandemic.
				\begin{enumerate}
					\item {\color{ForestGreen}Inequality}: ``Working from home'' is a regressive policy (most workers had to ride the bus).
...
\begin{enumerate}
	\item Turku is one of the {\color{ForestGreen}leading research-oriented institutions} in Finland.
		\begin{itemize}
			...
			\item Has the {\color{ForestGreen}PCRClab} lab ({\color{ForestGreen}FIRIPO}): really interesting to implement both {\bf survey} and {\bf lab} experiments!
			\item Has the {\color{ForestGreen}Participation in Long-Term Decision-Making} (PALO): doing interesting work implementing natural experiments related to ``direct democracy.'' Interesting! {\bf Would love to contribute!}
		\end{itemize}
	\item {\color{ForestGreen}Multidisciplinary-oriented} institution, as a whole: diversity is great!
	\item Finland has an outstanding educational system: would be perfect for my 3YO and 4YO children! (both German nationals).
\end{enumerate}
\end{frame}
...
\begin{itemize}
	\item I work on political economy: inequality and democracy.
	\item Experience designing, implementing, and analyzing experiments (survey, lab, natural).
	\item I'd be very happy to move to Turku and work with you.
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\item Then using simple OLS methods we run individual regressions between the vote-selling question and every $w$.
\end{itemize}
...
\column{0.5\textwidth}
	\begin{figure}[H]
		\vspace{-1cm}
		\hspace{-7mm}
		\includegraphics[scale=0.35]{/Users/hectorbahamonde/Dissertation/Papers/Earthquake_Paper/figure/simulation:plots-1.pdf}
	\end{figure}
\end{columns}
\end{frame}
